...
# CRITICAL: Validate CUDA availability - NO CPU FALLBACKS ALLOWED
if not torch.cuda.is_available():
    logger.error("❌ CUDA is not available! This application requires GPU acceleration.")
    logger.error("❌ Please ensure:")
    logger.error("   1. NVIDIA GPU is properly configured")
    logger.error("   2. NVIDIA drivers are installed")
    logger.error("   3. CUDA runtime is available")
    logger.error("   4. Container has GPU access")
    sys.exit(1)
...
logger.info("🚀 Forced PyTorch to use CUDA device 0")
