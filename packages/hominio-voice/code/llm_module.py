...
# OpenAI for RedPill API
try:
    from openai import OpenAI, APIError, APITimeoutError, RateLimitError, APIConnectionError
    OPENAI_AVAILABLE = True
except ImportError:
    OPENAI_AVAILABLE = False
    logging.warning("🤖⚠️ openai library not installed. OpenAI/RedPill backend will not function.")
    if sys.version_info >= (3, 9): 
        OpenAI = Any | None
    else: 
        OpenAI = Optional[Any]
    # Define dummy exception classes for type hints
...
# RedPill API Configuration (Phala/Llama 3.2)
REDPILL_API_KEY = os.getenv("REDPILL_API_KEY")
REDPILL_BASE_URL = os.getenv("REDPILL_BASE_URL", "https://api.red-pill.ai/v1")
...
# Removed Ollama connection checking and subprocess functions - only using RedPill API

# --- LLM Class ---
class LLM:
    """
    Provides interface for interacting with RedPill OpenAI API.

    Handles client initialization, streaming generation, request cancellation,
    and system prompts with optimized concurrency support.
    """
    ...
    def __init__(
        ...
        Args:
            backend: Must be "openai" (uses RedPill API by default).
            model: The identifier for the specific model to use.
            system_prompt: An optional system prompt to prepend to conversations.
            api_key: API key for RedPill/OpenAI API.
            base_url: Optional base URL (defaults to RedPill API endpoint).
            no_think: Experimental flag for future prompt modification.
        ...
        # Use RedPill API configuration
        self.effective_openai_key = self._api_key or REDPILL_API_KEY
        self.effective_openai_base_url = self._base_url or REDPILL_BASE_URL
    ...
    def _lazy_initialize_clients(self) -> bool:
        ...
        Creates the OpenAI SDK client with optimized connection pooling.
        ...
        with self._client_init_lock:
            ...
            logger.debug(f"🤖🔄 Lazy initializing OpenAI client")
            init_ok = False
            ...
            try:
                ...
                if init_ok:
                    ...
                    logger.error(f"🤖💥 OpenAI client initialization failed.")
            except Exception as e:
                logger.exception(f"🤖💥 Critical failure during OpenAI client initialization: {e}")
                init_ok = False
            finally:
    ...
    def prewarm(self, max_retries: int = 1) -> bool:
        ...
        if not self._lazy_initialize_clients():
            logger.error("🤖🔥💥 Prewarm failed: Could not initialize OpenAI client.")
            return False
        ...
        while attempts <= max_retries:
            ...
            try:
                ...
                if isinstance(e, RuntimeError) and "client failed" in str(e):
                    logger.error("   (This indicates the OpenAI client initialization failed)")
    ...
    def generate(
        ...
        **kwargs: Any
        ...
        # Lazy initialization for OpenAI client
        if not self._lazy_initialize_clients():
            raise RuntimeError(f"OpenAI client failed to initialize.")
        ...
        try:
            ...
            logger.debug(f"🤖💬 [{req_id}] Sending OpenAI/RedPill request")
        ...
        # Catch specific exceptions first
             ...
             logger.error(f"🤖💥 API Error during generation for {req_id}: {e}", exc_info=False)
    ...
    def _yield_openai_chunks(self, stream, request_id: str) -> Generator[str, None, None]:
        """
        Iterates over an OpenAI/RedPill stream, yielding content chunks.
        ...
        try:
            for chunk in stream:
                ...
                if chunk.choices:
                    ...
                    if content:
                        token_count += 1
                        yield content
            logger.debug(f"🤖✅ [{request_id}] Finished yielding {token_count} OpenAI/RedPill tokens.")
             ...
             if is_cancelled:
                  logger.warning(f"🤖⚠️ OpenAI/RedPill stream connection error likely due to cancellation for {request_id}: {e}")
             else:
                  logger.error(f"🤖💥 OpenAI/RedPill API connection error during streaming ({request_id}): {e}")
                  raise ConnectionError(f"OpenAI/RedPill communication error during streaming: {e}") from e
        except APIError as e:
            logger.error(f"🤖💥 OpenAI/RedPill API error during streaming ({request_id}): {e}")
            ...
            with self._requests_lock:
                is_cancelled = request_id not in self._active_requests
            if is_cancelled:
                logger.warning(f"🤖⚠️ OpenAI/RedPill stream error likely due to cancellation for {request_id}: {e}")
            else:
                logger.error(f"🤖💥 Unexpected error during OpenAI/RedPill streaming ({request_id}): {e}", exc_info=True)
                raise # Reraise for generate() to handle
        finally:
            ...
            if stream and hasattr(stream, 'close') and callable(stream.close):
                try:
                    logger.debug(f"🤖🗑️ [{request_id}] Closing OpenAI/RedPill stream in _yield_openai_chunks finally.")
                    stream.close()
                except Exception as close_err:
                    logger.warning(f"🤖⚠️ [{request_id}] Error closing OpenAI/RedPill stream in finally: {close_err}", exc_info=False)

# Removed _yield_ollama_chunks method - only using OpenAI/RedPill API
    ...
    def measure_inference_time(
        ...
        **kwargs: Any
        ...
        # Ensure client is ready (handles lazy init)
        if not self._lazy_initialize_clients():
            logger.error(f"🤖⏱️💥 Measurement failed: Could not initialize OpenAI client.")
            return None
...
if __name__ == "__main__":
    ...
    main_logger.info("🤖🚀 --- Running RedPill OpenAI API Example ---")

    # --- RedPill OpenAI Example ---
    try:
        # Get model from environment or use default
        model_name = os.getenv("LLM_MODEL", "llama-3.2-3b-instruct")
        
        main_logger.info(f"\n🤖⚙️ --- Initializing RedPill OpenAI API ({model_name}) ---")
        redpill_llm = LLM(
            backend="openai",
            model=model_name,
        ...
        main_logger.info("🤖🔥 --- Running RedPill Prewarm ---")
        prewarm_success = redpill_llm.prewarm(max_retries=1)

        if prewarm_success:
            main_logger.info("🤖✅ RedPill Prewarm/Initialization OK.")

            # Run inference time measurement
            main_logger.info("🤖⏱️ --- Running RedPill Inference Time Measurement ---")
            inf_time = redpill_llm.measure_inference_time(num_tokens=10, temperature=0.1)
            ...
            main_logger.info("🤖▶️ --- Running RedPill Generation via Context ---")
            try:
                with LLMGenerationContext(redpill_llm, "What is the capital of France? Respond briefly.") as generator:
                    print("\nRedPill Response: ", end="", flush=True)
                    response_text = ""
                    for token in generator:
                        print(token, end="", flush=True)
                        response_text += token
                    print("\n")
                main_logger.info("🤖✅ RedPill generation complete.")

            except (ConnectionError, RuntimeError, Exception) as e:
                if isinstance(e, ConnectionError):
                    main_logger.error(f"🤖💥 RedPill Connection Error: {e}")
                    main_logger.error("   🤖🔌 Please check your REDPILL_API_KEY and network connection.")
                else:
                    main_logger.error(f"🤖💥 RedPill Generation Error: {e}", exc_info=True)

        else:
            main_logger.error("🤖❌ RedPill Prewarm/Initialization Failed.")

    except (ImportError, ValueError, Exception) as e:
        main_logger.error(f"🤖💥 Failed to initialize RedPill API: {e}", exc_info=True)

    main_logger.info("\n" + "="*40)
    main_logger.info("🤖🏁 --- RedPill LLM Module Example Finished ---")